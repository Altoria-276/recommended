# 替换 NeuralMF 为 LightGCN（使用评分回归损失训练）
import numpy as np
import pandas as pd
import re
import math
...
class LightGCN:
    ...
    def _evaluate(self, data: pd.DataFrame) -> float:
        ...
        for _, row in data.iterrows():
            ...
            if u in self.user_map and i in self.item_map:
                pred = np.dot(self.user_embeddings[self.user_map[u]], self.item_embeddings[self.item_map[i]])
            else:
    ...
    def fit(self, train_data: pd.DataFrame, val_data: Optional[pd.DataFrame] = None, test_data: Optional[pd.DataFrame] = None):
        ...
        # 构建邻接矩阵
        adj = [[] for _ in range(n_users + n_items)]
        for _, row in train_data.iterrows():
            u, i = self.user_map[row['user']], self.item_map[row['item']]
            adj[u].append(n_users + i)
            adj[n_users + i].append(u)

        for epoch in trange(self.n_epochs, desc="训练进度", disable=not self.verbose):
            # Embedding 传播
            emb_user = self.user_embeddings.copy()
            emb_item = self.item_embeddings.copy()
            all_emb = np.concatenate([emb_user, emb_item], axis=0)
            emb_layers = [all_emb]
            for _ in range(self.n_layers):
                next_emb = np.zeros_like(all_emb)
                for idx, neighbors in enumerate(adj):
                    if neighbors:
                        next_emb[idx] = np.mean(all_emb[neighbors], axis=0)
                emb_layers.append(next_emb)
                all_emb = next_emb
            ...
            # 使用评分回归的 loss 来更新嵌入
            grad_user = np.zeros_like(self.user_embeddings)
            grad_item = np.zeros_like(self.item_embeddings)
            for _, row in train_data.iterrows():
                ...
                error = pred - rating
                grad_user[u_idx] += error * self.item_embeddings[i_idx] + self.reg * self.user_embeddings[u_idx]
                grad_item[i_idx] += error * self.user_embeddings[u_idx] + self.reg * self.item_embeddings[i_idx]

            grad_user = np.clip(grad_user, -self.grad_clip, self.grad_clip)
            grad_item = np.clip(grad_item, -self.grad_clip, self.grad_clip)
            self.user_embeddings -= self.lr * grad_user
            self.item_embeddings -= self.lr * grad_item
            ...
            trange(self.n_epochs).set_postfix(postfix)
    ...
    def predict(self, data: pd.DataFrame) -> List[float]:
        ...
        for _, row in data.iterrows():
            ...
            if u in self.user_map and i in self.item_map:
                p = np.dot(self.user_embeddings[self.user_map[u]], self.item_embeddings[self.item_map[i]])
            else:
                p = self.global_mean
            preds.append(max(self.rating_scale[0], min(self.rating_scale[1], p)))
        return preds

# 其它部分（parse_data、save_predictions、save_training_stats、main）保持不变即可

def save_training_stats(stats_path: Path, model: LightGCN, args: argparse.Namespace) -> None:
    with stats_path.open('w', encoding='utf-8') as f:
        f.write("===== 训练统计信息 =====\n\n")
        f.write(f"模型: LightGCN\n")
